...
elliott = converter.parse('/Users/Daniel/OneDrive/FSU/Fall 2015/Seminar/Final Project/Prolation tests/Elliott.xml')



# prolation_amount = [.3333333, .5, .66666666, 1, 1.5, 2, 3]
# prolation_amount = [1, 1.25, 1.5, 1.75, 2, .75, .5]
prolation_amount = [1, 2, .5, 1.5]
# offset_amount = [0, .5, 1, 1.5, 2, 2.5, 3, 3.5, 4]
# offset_amount = [0, 1, 2, 3, 4]
transposition_amount = [1, 2, 3, 4, 5, 6, 7, 8]
...
def comparison(songs):
    ...
    pl.ylabel('percent canon success')
...
def resolve_diss(intervals):
    ...
    for x in range(1, len(intervals)):
        if intervals[x - 1] in dissonant and intervals[x] in dissonant:
            checker += 1
            print('double d trashed')
        else:
            print('CONSONANCE!')
        if intervals[x - 1] == 6 and intervals[x] != (4 or 3 or 9 or 8):
            checker += 1
