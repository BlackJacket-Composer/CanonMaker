...
def multi_tester(melody):
    ...
    for p in alls:
        ...
        if parallel_checker(tests) == True:
            # all_dict[p] = True
            trues.append(p)
            p.show()
...
def parallel_checker(intervals):
    ...
    checker = 1
    for x in range(1, len(final_ints)):

        if final_ints[x] in dissonant and final_ints[x-1] in dissonant:
            checker += 1
            print('dissonant')

        elif final_ints[x - 1] == 6 and final_ints[x] != (4 or 3 or 9 or 8):
            checker += 1
            print("bad tritone resolution")

        elif final_ints[x - 1] == 5 and final_ints[x] != (4 or 3 or 7 or 8 or 9):
            checker += 1
            print('bad 4th')

        elif final_ints[x - 1] in seconds and final_ints[x] != (4 or 3 or 0):
            checker += 1
            print('bad second')

        elif final_ints[x - 1] == sevenths and final_ints[x] != (0 or 9 or 8):
            checker += 1
            print("bad 7, bad 7")

        elif final_ints[x] == 7 and final_ints[x - 1] == 7:
            # print "parallel fifths"
            checker += 1
            parallel_fifths += 1

        elif final_ints[x] == 0 and final_ints[x - 1] == 0:
            # print "parallel octaves"
            checker += 1
            parallel_octaves += 1

    for x in range(3, len(final_ints)):
        if final_ints[x - 3] in thirds and final_ints[x - 2] in thirds and final_ints[x - 1] in thirds \
                and final_ints[x] in thirds:
            checker += 1

            # print "3 motion too similar"
        elif final_ints[x - 3] in sixths and final_ints[x - 2] in sixths and final_ints[x - 1] in sixths \
                and final_ints[x] in sixths:
            checker += 1
